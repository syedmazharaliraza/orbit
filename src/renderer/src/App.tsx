...
function WorkerDetail({ worker, onClose }: { worker: Worker; onClose: () => void }) {
  ...
  return <section className={`worker-detail worker-detail--${worker.state}`}><header className="detail-header"><div className={`detail-avatar detail-avatar--${worker.state}`}><Astronaut size={56} hue={worker.hue} mark={worker.mark} state={worker.state} /></div><div><div className="detail-title"><h1>{worker.name}</h1><span className={`detail-status detail-status--${worker.presentation}`}>{presentationLabel(worker.presentation)} · {worker.elapsed}</span></div><p>{worker.task}</p></div><button className="detail-close" onClick={onClose} aria-label="Back to orbit">×</button></header><div className="detail-chips"><span>{worker.repo || 'repository unavailable'}</span>{worker.branch && <span>{worker.branch}</span>}<span>{worker.model}</span><span>effort {worker.effort}</span><span>{worker.priority}</span></div><section className="detail-current"><div><span className={worker.fileTag === 'EDITING' ? 'tag tag--edit' : 'tag'}>{worker.file ? worker.fileTag : 'ACTIVITY'}</span><b>{worker.file || worker.action || 'No current activity observed'}</b><small>{worker.path}</small><em>{worker.edit}</em></div>{worker.tools?.length ? <div className="observed-row"><label>TOOLS</label>{worker.tools.map(tool => <span key={tool.name}>{tool.name.toLowerCase()} ×{tool.count}</span>)}</div> : null}{worker.relevantFiles?.length ? <div className="observed-row"><label>FILES</label>{worker.relevantFiles.slice(0, 6).map(file => <span key={file}>{file.split('/').pop() || file}</span>)}</div> : null}</section>{worker.question && <QuestionNotice question={worker.question} />}{worker.permission && <PermissionNotice permission={worker.permission} />}{worker.waitingFor && <div className="waiting-note">{waitingLabel(worker.waitingFor)}</div>}<section className="detail-log"><div><label>ACTIVITY · {worker.activity.length}</label><code>{worker.activity.map((item, index) => <span key={`${item.observedAt || item.time}-${index}`} className={item.current ? 'activity-current green' : ''}>{item.current ? '› ' : ''}{item.time} {item.text}</span>)}</code></div><div><label>LAST MEANINGFUL OUTPUT</label><p>{worker.message ? `“${worker.message}”` : 'No meaningful assistant output observed yet.'}</p>{worker.usage && <small className="usage-copy">{formatUsage(worker.usage)}</small>}</div></section><div className="detail-budget"><span>{worker.elapsed}</span>{worker.budget !== undefined && <div><i style={{ width: `${worker.budget}%` }} /></div>}<span>{worker.context}</span>{worker.cost && <b>{worker.cost}</b>}</div>{worker.signal ? <DriftHint worker={worker} /> : <div className="detail-signal"><i />observation-only · no action is available here</div>}<footer><span>{attention ? 'Return to Claude Code to respond.' : 'Observed session · read-only'}</span></footer></section>
}
